...
void test_sidh()
{
  __m512i skA[4], pkA[6*NWORDS]; 
  __m512i skB[4], pkB[6*NWORDS];
  __m512i ssA[2*NWORDS], ssB[2*NWORDS];
  uint64_t pkA51[6*NWORDS], pkA64[6*NWORDS] = { 0 };
  uint64_t pkB51[6*NWORDS], pkB64[6*NWORDS] = { 0 };
...
void timing_sike()
{
  ...
  MEASURE_TIME(crypto_kem_keypair((uint8_t *)pk, (uint8_t *)sk), ITER_S);
  printf("* KEYGEN : %ld\n", diff_cycles);

  LOAD_CACHE(crypto_kem_enc((uint8_t *)ct, (uint8_t *)ssa, (uint8_t *)pk), ITER_S);
  MEASURE_TIME(crypto_kem_enc((uint8_t *)ct, (uint8_t *)ssa, (uint8_t *)pk), ITER_S);
  printf("* ENCAPS : %ld\n", diff_cycles);

  LOAD_CACHE(crypto_kem_dec((uint8_t *)ssb, (uint8_t *)ct, (uint8_t *)sk), ITER_S);
  MEASURE_TIME(crypto_kem_dec((uint8_t *)ssb, (uint8_t *)ct, (uint8_t *)sk), ITER_S);
  printf("* DECAPS : %ld\n", diff_cycles);
}
