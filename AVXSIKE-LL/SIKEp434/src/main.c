...
void test_fp()
{
  ...
  LOAD_CACHE(mp_mul_8x1w_v1(z_8x1w, a_8x1w, b_8x1w), ITER_M);
  MEASURE_TIME(mp_mul_8x1w_v1(z_8x1w, a_8x1w, b_8x1w), ITER_L);
  printf("* 8x1w INT MUL V1: %ld\n", diff_cycles);

  LOAD_CACHE(mp_mul_8x1w_v2(z_8x1w, a_8x1w, b_8x1w), ITER_M);
  MEASURE_TIME(mp_mul_8x1w_v2(z_8x1w, a_8x1w, b_8x1w), ITER_L);
  printf("* 8x1w INT MUL V2: %ld\n", diff_cycles);
...
void test_fpx()
{
  ...
  // 8x1x1w Fp2 mul 
  fp2mul_mont_8x1x1w(r_8x1x1w, a_8x1x1w, b_8x1x1w);
  ...
  // 4x2x1w Fp2 mul
  fp2mul_mont_4x2x1w(r_4x2x1w, a_4x2x1w, b_4x2x1w);
  ...
  // 2x2x2w Fp2 mul
  fp2mul_mont_2x2x2w(r_2x2x2w, a_2x2x2w, b_2x2x2w);
...
void test_curve()
{
  ...
  uint64_t vmont_Rx4[VNWORDS] = {
    0x000000E858D70, 0x0000000000000, 0x0000000000000, 0x0000000000000, 
    0x26D88C35B0000, 0x1B739BBDFB700, 0x7C7CFC4383EC9, 0x5F3AD6E5A15CB, 
    0x1725A5A };

  uint64_t vmont_Rx6[VNWORDS] = {
    0x0000015C85428, 0x0000000000000, 0x0000000000000, 0x0000000000000, 
    0x3A44D25088000, 0x692D699CF9280, 0x7ABB7A6545E2D, 0x0ED84258720B1, 
    0x22B8788 };
  ...
  Q_1x2x2x2w[i]      = VZERO;
  C24_A1_1x2x2x2w[i] = VSET(0, 0, 0, vmont_Rx4[i], 0, 0, 0, vmont_R[i]);
  ...
  P_1x2x2x2w[i]      = VSET(0, xP1[i], 0, xP0[i], 0, zP1[i], 0, zP0[i]);
  Q_1x2x2x2w[i]      = VZERO;
  C24_A1_1x2x2x2w[i] = VSET(0, 0, 0, vmont_Rx4[i], 0, 0, 0, vmont_Rx6[i]);
